...
/**
 * A GUI-based application to generate reports using JasperReports.
 * Users can enter details and generate reports dynamically.
 */
public class ExportReport {
    ...
    private static void generateReport(String jasperTemplate, JTextField nameField, JTextField schoolYearField,
                                       JTextField strandField, JTextField trackField, JTextField purposeField,
                                       JTextField dateGivenField, JComboBox<String> signerComboBox,
        ...
        if (action.equals("print")) {
            reportGenerator.printReport(jasperPrint);
        } else {
            JFileChooser fileChooser = new JFileChooser();
            fileChooser.setSelectedFile(new File(outputName + (action.equals("docx") ? ".docx" : ".pdf")));
            if (fileChooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
                if (action.equals("docx")) {
                    reportGenerator.exportToDocx(jasperPrint, fileChooser.getSelectedFile());
                } else {
                    reportGenerator.exportToPdf(jasperPrint, fileChooser.getSelectedFile());
                }
            }
        }
    }
